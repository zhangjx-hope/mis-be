...
import org.jeecg.modules.system.controller.AESUtils;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	// 操作日志签名
	public byte[] signLog(PrivateKey privateKey, String log) throws NoSuchAlgorithmException, InvalidKeySpecException, Exception {
		Signature sign = Signature.getInstance("SHA256withRSA");
		sign.initSign(privateKey);
		sign.update(log.getBytes());
		return sign.sign();
	...
	// 验证日志签名
	public boolean verifyLog(String username, String log, byte[] signature) throws NoSuchAlgorithmException, InvalidKeySpecException, Exception {
		...
		byte[] publicKeyBytes = Base64.getDecoder().decode(sysUser.getPublicKeyBase64());
		PublicKey publicKey = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(publicKeyBytes));

		Signature verify = Signature.getInstance("SHA256withRSA");
		verify.initVerify(publicKey);
		verify.update(log.getBytes());
    ...
    public Result<?> resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) throws Exception {
        ...
        // 解密私钥
        byte[] encryptedPrivateKey = Base64.getDecoder().decode(user.getPrivateKeyBytes());
        byte[] privateBytes = AESUtils.decrypt(encryptedPrivateKey, oldpassword);


        // 加密新私钥
        String encryptedPrivateKeyNew = AESUtils.encrypt(privateBytes, newpassword);


        // 更新数据库中的私钥
        this.userMapper.update(new SysUser().setPassword(newpassword).setPrivateKeyBytes(encryptedPrivateKeyNew), new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, user.getId()));

		// 操作日志签名示例

		// 解密私钥 私密与密码相同
		PrivateKey privateKey = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(privateBytes));
		String log = "用户" + user.getUsername() + "修改密码";
		byte[] signature = signLog(privateKey, log);
		if (signature != null) {
			System.out.println("修改密码日志签名成功");
		} else {
			System.out.println("修改密码日志签名失败");
		}

		//验证签名
		Boolean flag = false;
		flag = verifyLog(user.getUsername(), log, signature);
		if (flag) {
			System.out.println("修改密码日志签名验证成功");
		} else {

			System.out.println("修改密码日志签名验证失败");
		}
